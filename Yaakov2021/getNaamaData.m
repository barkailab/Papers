...
elseif ip.Results.figure==23 %% look at spike in experiments
    allProfiles=struct2table([
        dir('/home/labs/barkailab/felixj/gilad/*h3x2*.mat');
        dir('/home/labs/barkailab/felixj/gilad/*h32x*.mat');
        ])
...
if ismember(ip.Results.figure,[1,2]) 
    intTags=find(~contains(allTags,{'h1','z','sup'},'IgnoreCase',true))
    intProfiles=ismember(allProfiles.ab,{'myc','ha'})  & ismember(allProfiles.tagid,intTags);
...
elseif ip.Results.figure==23
    intProfiles=contains(allProfiles.gt,{'wt','rtt','sml'})&contains(allProfiles.ab,{'ha','myc'})&contains(allProfiles.exp,{'31','35','36'})&~contains(allProfiles.ab,{'f'});%true(size(allProfiles.ab));
end
...
smeta.tag2(contains(smeta.tag,'h32x'))={'h3x2'};
