...
highOcc=find(mean(movmean(nucData(:,strcmp(smeta.ab,'ha')& ~contains(smeta.tag2,'par')& strcmp(smeta.gt,'wt') &smeta.bad==0),5)>30,2)>0.05);
highOcc=highOcc(highOcc>2);
nucData(unique(acol(highOcc+[-2:2])),:)=NaN;
lowOcc=find(mean(movmean(nucData(:,strcmp(smeta.ab,'ha') & ~contains(smeta.tag2,'par')&strcmp(smeta.gt,'wt') &smeta.bad==0),3)<0.01,2)>0.05);
...
labelSmp=find(contains(smeta.gt,'rlf2'))%find(contains(smeta.con,{'async','alpha'}));%true(size(nucData,2),1))%find(contains(smeta.tag2,{'h3x2','h3y2'})&contains(smeta.con,{'async','alpha'}));
yticks(labelSmp)
